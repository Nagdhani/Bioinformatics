...
for(h in 1:length(SeqData$V1)){
  if(SeqData$V5[h]==SeqData$V5[h+2] && SeqData$V8[h] == 0 && is.na(SeqData$V8[h])==FALSE && SeqData$V5[h]!= ""){
...
```
Removing periods before start of DATA
```{r}
...
write.table(CompleteDF,'/Users/nagendra/Desktop/Bioinformatics/Project/Data/EditedHIVData.csv', sep = ",")
```

Updating blast sequence
```{r}
BLAST <- read.table('/Users/nagendra/Desktop/Bioinformatics/Project/Data/FullBlast_comp.csv', sep= ",")
```
...
```{r}
BLASTCompare <- data.frame(PatientName, SimilarityPct, BPlength)
```
