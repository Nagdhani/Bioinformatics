...
output: pdf_document
---
#chunk of code that defines all the packages that you need and installs thema
...
## 1 Plotting IRange Data ****ON TEST****
...
This first chunk uses the 'readLines()' function, reading each line verbatim
```{r, echo=FALSE}
...
#con opens file and opens for reading
con <- file('//Users//nagendra//Desktop//Bioinformatics//dubjay-bioinformatics//Chapters9-10//egfr_flank.fasta',open="r")
# converts each line into an index (not useful for larger files but is for small ones, not useful for fasta with line breaks)
line <- readLines(con) 

# Go line-by-line, getting the name and sequence, saving in appropriate vector
# If line starts with >, then add line to the name vector, else if skip those lines. If not starting with carrot or empty, it is sequence data so add to sequence vector
...
df <- read.table('//Users//nagendra//Desktop//Bioinformatics//dubjay-bioinformatics//Chapters9-10//egfr_flank.fasta',header=FALSE,sep="|",fill=TRUE)
```

The final method is specific for FASTA files, using the 'Biostrings' library and 'readDNAStringSet()'. Most specialized can sometimes mean least robust, thoughh also less prone to error.
```{r}
# Upload library
library("Biostrings")

# Read in FASTA file
fastaFile <- readDNAStringSet('//Users//nagendra//Desktop//Bioinformatics//dubjay-bioinformatics//Chapters9-10//egfr_flank.fasta')
...
3. transforming the sequence scores from their chracter format to their PHRED scores
```{r,echo=FALSE}

# Upload FASTQ file, identify indices of name, sequence, and score information
fastq <- read.table('//Users//nagendra//Desktop//Bioinformatics//dubjay-bioinformatics//Chapters9-10//contam.fastq',header=FALSE,sep="|",fill=TRUE)
#create sequence that starts at 1 and ends at the end of the column. Jump 4 lines each time you add an entry
...
#vectors can be used to index. THe [name] stands for the numbers of the rows that you are adding to a vector
Name <- fastq$V1[name]
